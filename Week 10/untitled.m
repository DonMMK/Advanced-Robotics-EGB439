...
function [x,S] = update_step(map,z,x,S,Q)
    ...
    for i = 1 : length(map) % for the length of the map
        ...
        Bearing = atan2(map(i,2) - x(2) , map(i,1) - x(1)) - x(3); 
        
        % the G matrix
        row1col1 = -(map(i,1)-x(1))/range;
        row1col2 = -(map(i,2)-x(2))/range;
        row2col1 = (map(i,2)-x(2))/range^2;
        row2col2 = -(map(i,1)-x(1))/range^2;
        ...
        KPart2 = G*S*G' + Q;
        K_t = KPart1 * (KPart2)^-1;
        err = (z(i,:) - [range , wrapToPi(Bearing)]);

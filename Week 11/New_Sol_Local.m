...
% the main stuff in parts
% the order of the m_est 
...
function [mu, Sigma] = initLandmarks(z,Q,mu,Sigma)
    ...
    for i = 1:length(z)
        ...
        elseif Range == 0 && LengthOfMU ~= 3
            ...
            NUMM2 = 2;
            NUM3 = 3;
            lnew = [xr + Range * cos(theta_r + Beearing); 
                yr + Range * sin(theta_r + Beearing)];
            mu(NUMM2 * i + NUMM2 : NUMM2 * i + NUM3) = lnew;

            LZZ1 = cos(theta_r + Beearing);
            LZZ2 = -Range*sin(theta_r + Beearing);
            LZZ3 = sin(theta_r + Beearing) ;
            LZZ4 = Range*cos(theta_r + Beearing);
            Lz = [ LZZ1 , LZZ2; 
                 LZZ3,LZZ4 ;];
            multipli1 = Lz*Q;
...
function [mu, Sigma] = update_step(landmarkID,zi,Q,mu,Sigma)
    ...
    Gmat3 = -(yl-yr)/r^2;
    Gmat4 = (xl-xr)/r^2;
    G = [ Gmat1,  Gmat2   ; Gmat3  , Gmat4];
    
    zerooarray = zeros(2,(i-1)*2);
    ZeroArrayHold = zerooarray;
    
    teemp = zeros(2,(landmarks_num-i)*2);
    ...
    insidemat2 = -(yl-yr)/r;
    insidemat3 = 0 ;
    insidemat4 = (yl-yr)/r^2;
    insidemat5 = -(xl-xr)/r^2 ;
    insidemat6 = -1;
    ...
    insidee1 = (xr-xl)^2;
    insidee2 = (yr-yl)^2;
    insidee3 = atan2(yl-yr, xl-xr);
    
    h = [ sqrt(insidee1 + insidee2);
        insidee3 - theta_r];
